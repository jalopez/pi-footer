...
export function fieldsForWidget(widget) {
    ...
    for (const property of properties) {
        if (property.options?.showInFields === false)
            continue;
        if (!isMetadataPropertyVisible(property, widget.options))
            continue;
        fields.push(metadataField(property));
    }
    return fields;
}
export function colorFields(widget) {
    const fields = [
    ...
    if (widget &&
        ...
        fields.push({ id: "warningFg", label: "Warning foreground", kind: "color" }, { id: "warningBg", label: "Warning background", kind: "color" }, { id: "warningFgAnsi", label: "Custom ANSI256 warning foreground", kind: "ansi" }, { id: "warningBgAnsi", label: "Custom ANSI256 warning background", kind: "ansi" }, { id: "dangerFg", label: "Danger foreground", kind: "color" }, { id: "dangerBg", label: "Danger background", kind: "color" }, { id: "dangerFgAnsi", label: "Custom ANSI256 danger foreground", kind: "ansi" }, { id: "dangerBgAnsi", label: "Custom ANSI256 danger background", kind: "ansi" });
    }
    return fields;
}
export function fieldValue(widget, field) {
    if (field.id === "enabled")
        return widget.enabled ? "on" : "off";
    if (field.kind === "boolean")
        return getBooleanField(widget, field.id) ? "on" : "off";
    if (field.kind === "number")
        return String(getNumberField(widget, field.id) ?? "");
...
export function colorFieldValue(widget, field) {
    if (field.id === "bold")
        return widget.options.bold ? "on" : "off";
    if (field.id === "fg")
        return colorDisplayName(widget.options.fg);
    if (field.id === "bg")
        return colorDisplayName(widget.options.bg);
    if (field.id === "warningFg")
        return colorDisplayName(colorOption(widget, "warningFg"));
    if (field.id === "warningBg")
        return colorDisplayName(colorOption(widget, "warningBg"));
    if (field.id === "dangerFg")
        return colorDisplayName(colorOption(widget, "dangerFg"));
    if (field.id === "dangerBg")
        return colorDisplayName(colorOption(widget, "dangerBg"));
    if (field.id === "fgAnsi")
...
export function getBooleanField(widget, id) {
    const spec = registry.spec(widget.type);
    const property = spec.properties.find((item) => item.id === id);
    if (property?.kind === "boolean")
        return Boolean(widget.options[id] ?? property.default);
    if (spec.baseOptions.some((option) => option === id)) {
        if (id === "raw")
            return widget.options.raw ?? false;
        if (id === "hideWhenEmpty")
            return widget.options.hideWhenEmpty ?? false;
        if (id === "hideWhenZero")
            return widget.options.hideWhenZero ?? false;
    }
    return false;
}
export function getNumberField(widget, id) {
...
export function getTextField(widget, id) {
    const spec = registry.spec(widget.type);
    const property = spec.properties.find((item) => item.id === id);
    ...
    if (spec.baseOptions.some((option) => option === id)) {
        if (id === "icon")
            return widget.options.icon ?? "";
        if (id === "text")
            return widget.options.text ?? "";
    }
    return "";
}
export function formatWidgetOptions(widget) {
    const spec = registry.spec(widget.type);
    const properties = spec.properties;
    ...
    if (spec.baseOptions.some((option) => option === "text") &&
    ...
    for (const property of properties) {
        if (!isMetadataPropertyVisible(property, widget.options))
            continue;
        if (property.options?.showInWidgets === false)
            continue;
        addMetadataSummaryPart(parts, widget, property);
    }
    return parts.join(" • ");
}
export function formatWidgetColorOptions(widget) {
...
export function isMetadataPropertyVisible(property, options) {
    const condition = property.showWhen;
    if (!condition)
        return true;
    return options[condition.property] === condition.equals;
}
function colorOption(widget, key) {
    return normalizeColor(widget.options[key]);
}
function formatMetadataWidgetColorOptions(widget) {
    const spec = registry.spec(widget.type);
    const properties = spec.properties;
    const parts = baseSummaryParts(widget);
    if (spec.baseOptions.some((option) => option === "text") &&
    ...
    for (const property of properties) {
        if (!isMetadataPropertyVisible(property, widget.options))
            continue;
        if (!property.options?.showInColors)
            continue;
        addMetadataSummaryPart(parts, widget, property);
    }
    return parts.join(" • ");
...
function metadataField(property) {
    ...
    if (property.options?.min !== undefined)
        field.min = property.options.min;
    if (property.options?.max !== undefined)
        field.max = property.options.max;
    if (property.options?.choices !== undefined)
        field.choices = property.options.choices;
    if (property.options?.editAction !== undefined)
        field.editAction = property.options.editAction;
    return field;
}
function addMetadataSummaryPart(parts, widget, property) {
    const value = widget.options[property.id] ?? property.default;
    if (property.kind === "boolean") {
        if (value === true)
            parts.push(property.options?.label ?? property.id);
        return;
    }
    ...
    if (property.kind === "number" || property.kind === "text") {
        ...
        if (value !== undefined && value !== "")
            parts.push(`${prefix}=${renderedValue}`);
    }
}
function baseSummaryParts(widget) {
    const { options } = widget;
    const spec = registry.spec(widget.type);
    const parts = [];
    if (spec.baseOptions.some((option) => option === "raw") && options.raw)
        parts.push("raw");
