...
export class AddWidgetScreen extends Controller {
    ...
    renderScreen(width) {
        ...
        const start = clamp(this.selected - Math.floor(visibleCount / 2), 0, Math.max(0, items.length - visibleCount));
        const visible = items.slice(start, start + visibleCount);
        const lines = [
            this.render.line(this.render.menuTitle("Add Widget", "Search or select a widget to add"), width),
            this.render.line(this.ctx.theme.dim(this.hint()), width),
            this.render.line(this.ctx.theme.dim(this.countLabel(items.length, start, visible.length)), width),
        ];
    ...
    filterWidgets() {
        ...
        return registry.definitions.filter((definition) => `${definition.category} ${definition.label} ${definition.type}`
            .toLowerCase()
