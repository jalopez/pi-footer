...
export class EditWidgetScreen extends Controller {
    ...
    renderScreen(width) {
        ...
        const lines = [
            this.render.line(this.render.menuTitle(this.widgetTitle(widget), "Select a field to edit its options"), width),
            this.render.line(this.ctx.theme.dim(HINT), width),
            ...fields.map((field, index) => this.render.menuLine(index === this.selected, `${field.label}: ${fieldValue(widget, field)}`, width)),
        ];
        if (selectedField?.editAction === "external-status-key") {
            lines.push(...statusKeyPickerLines(this.ctx.getExtensionStatuses, this.ctx.state.store.settings.extensionStatusRow, width, (content, lineWidth) => this.render.line(content, lineWidth), (value) => this.ctx.theme.dim(value)));
        }
        if (widget.type === "event") {
            lines.push(...eventWidgetUsageLines(widget, width, (content, lineWidth) => this.render.line(content, lineWidth), (value) => this.ctx.theme.dim(value)));
        }
        return lines;
    }
    handleInput(data) {
    ...
    applyTextFieldEdit(widget, field, edit) {
        if (applyOptionFieldEdit(widget, field, edit) !== "changed")
            return;
        this.ctx.emitChange();
    }
    adjustField(widget, field, delta) {
        const result = applyOptionField(widget, field, delta);
        if (result === "external-status-key") {
            if (!cycleExternalStatusKey(widget, this.ctx.getExtensionStatuses, this.ctx.state.store.settings.extensionStatusRow, delta))
                return;
        }
