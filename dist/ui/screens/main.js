...
export class MainScreen extends Controller {
    ...
    renderScreen(width) {
        return [
            this.render.line(this.render.menuTitle("Main Menu", "Configure any number of status lines with various widgets"), width),
            this.render.line(this.ctx.theme.dim(HINT), width),
            ...ITEMS.map((item, index) => this.render.menuLine(index === this.selected, `${item.label} ${this.ctx.theme.dim(item.description)}`, width)),
        ];
    }
    handleInput(data) {
    ...
    applySelectedAction() {
        ...
        if (!action)
            return;
        if (action.type === "view")
            this.ctx.show(action.view);
        else if (action.type === "save-exit")
            this.ctx.save(true);
        else
            this.ctx.exitWithoutSaving();
    }
}
