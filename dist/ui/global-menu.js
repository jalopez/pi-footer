import { ansi256Digits, appendAnsi256Digit, colorDisplayName, deleteAnsi256Digit, STANDARD_COLORS, } from "../colors.js";
...
export const GLOBAL_MENU_HINT = "↑/↓ option • ←/→ or enter change • type digits for ANSI256 • backspace delete • esc back";
export function globalMenuFields(config) {
    return [
...
export function globalMenuAction(index) {
    return GLOBAL_MENU_ACTIONS[index] ?? "reset";
}
export function applyGlobalMenuAction(config, action, delta) {
    if (action === "preset")
        return configWithPreset(config, cycle(Object.keys(PRESET_DEFINITIONS), config.preset, delta));
    if (action === "reset")
...
export function applyGlobalSettingsAction(settings, action, delta) {
    if (action === "toggle-enabled")
        ...
        settings.minimalist = !settings.minimalist;
    else
        return false;
    return true;
}
export function applyGlobalSettingsTextInput(settings, action, data) {
    if (!/^\d$/.test(data))
        return false;
...
export function applyGlobalSettingsBackspace(settings, action) {
    if (action === "separator-fg-ansi")
...
function cycleStandardColor(current, delta) {
    return cycle(STANDARD_COLORS.map((color) => color.value), current, delta);
}
