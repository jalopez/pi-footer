...
export function previewTitleParts(innerWidth) {
...
export function configTitleBarParts(innerWidth, dim, now = Date.now(), status = "", border = (text) => text) {
    ...
    const separator = border("─".repeat(Math.max(1, contentWidth - visibleWidth(left) - statusWidth)));
    return {
...
export function retroText(text, now = Date.now()) {
