...
export const TokensWidget = defineWidget({
    ...
    render({ ctx, options, renderWidget }) {
        return renderWidget(`↑${formatTokenCount(ctx.metrics.inputTokens, options.tokenFormatStyle)} ↓${formatTokenCount(ctx.metrics.outputTokens, options.tokenFormatStyle)}`);
    },
});
