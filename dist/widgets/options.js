...
export function defaultOptionsFromSpec(spec) {
    const base = {};
    ...
    if (dynamicDefaults)
        Object.assign(base, dynamicDefaults);
...
export function sanitizeOptionsFromSpec(spec, input) {
...
function sanitizeWidgetProperty(property, value) {
    switch (property.kind) {
        ...
        case "number":
            return sanitizeMetadataNumber(value, property.default, property.options?.min, property.options?.max);
        case "text":
            if (property.id === "warningFg" ||
