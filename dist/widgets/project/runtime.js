...
export const RuntimeWidget = defineWidget({
    ...
    render({ ctx, options, renderWidget }) {
        ...
        if (cwd.length === 0)
            return renderWidget("");
        const info = asyncCache.get(CACHE_NAMESPACES.runtime, `${cwd}:${options.displayVersion ? "version" : "no-version"}`, RUNTIME_CACHE_TTL_MS, { cwd, displayVersion: options.displayVersion }, detectRuntime, ctx.requestRender);
        if (!info)
            return renderWidget("", { icons: { emoji: "", nerd: "", text: "" } });
...
function renderRuntimeValue(info, displayVersion, style) {
    if (!displayVersion || !info.version)
        return info.name;
    if (style === "compact")
        return compactRuntimeVersion(info.name, info.version);
...
async function detectRuntime({ cwd, displayVersion, }) {
    const entries = await readCwdEntries(cwd);
    if (!entries)
        return null;
    for (const definition of RUNTIME_DEFINITIONS) {
        if (!matchesRuntime(definition, entries))
            continue;
...
function matchesRuntime(definition, entries) {
    if (hasAnyFile(entries, definition.files))
        return true;
...
async function runVersion(versionCommand) {
    return new Promise((resolve) => {
        execFile(versionCommand.command, [...versionCommand.args], { encoding: "utf8", timeout: VERSION_TIMEOUT_MS, windowsHide: true }, (error, stdout, stderr) => {
