...
import { cloneConfig, configWithPreset, DEFAULT_CONFIG, isPreset, loadConfig, saveConfig, STATUS_KEY, } from "./config.js";
import { EventWidgetValues, UPDATE_EVENT_WIDGET_EVENT } from "./event-widgets.js";
import { EMPTY_EXTENSION_STATUSES, visibleExtensionStatusRowEntries, } from "./extension-statuses.js";
...
export default async function statuslineExtension(pi) {
    ...
    function apply(ctx) {
        ...
        ctx.ui.setFooter((tui, theme, footerData) => {
            ...
            return {
                dispose() {
                    unsubscribeBranch();
                    if (renderCurrentFooter)
                        renderCurrentFooter = undefined;
                ...
                render(width) {
                    ...
                    if (lines.length === 0)
                        return [];
                    const statuses = visibleExtensionStatusRowEntries(footerData.getExtensionStatuses(), config.extensionStatusRow.hiddenKeys, STATUS_KEY).map((entry) => entry.value);
    ...
    pi.registerCommand("footer", {
        ...
        handler: async (args, ctx) => {
            ...
            if (handled)
                return;
            const previewData = collectStatuslineData(ctx, pi, {
                getGitBranch: () => "main",
            }, eventWidgets.values, {
                collectGit: true,
            ...
            const result = await openStatuslineConfigUi(ctx, config, previewData, (updated) => {
                replaceConfig(updated);
                apply(ctx);
            }, async (updated) => setConfig(updated, ctx), getExtensionStatuses);
...
async function handleArgs(args, ctx, currentConfig, setConfig) {
    ...
    ctx.ui.notify("Usage: /footer [on|off|reset|preset compact|default|powerline|powerline-bright|powerline-blocks|powerline-mono|git-heavy|pi-footer|demo|demo-standard]", "warning");
    return true;
}
