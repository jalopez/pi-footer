...
export function renderStatuslines(store, data, width, options = {}) {
...
function renderLine(line, settings, width, ctx) {
    const rendered = line
        ...
        .map((widget) => ({
        widget,
        segment: widget.render(contextForDependencies(ctx.baseCtx, registry.spec(widget.type).dependencies, ctx.data, ctx.options)) ?? "",
    }));
...
function effectiveWidth(settings, width) {
    if (settings.terminal.widthMode === "full-minus-40")
        return Math.max(1, width - 40);
    return width;
}
function joinSegments(entries, settings) {
    ...
    for (let index = 1; index < segments.length; index += 1) {
        ...
        if (previous.widget.type !== "separator" && current.widget.type !== "separator") {
            output += applyColors(separatorText(settings.separator), settings.separatorFg, settings.separatorBg, false, settings.terminal.colorLevel);
        }
