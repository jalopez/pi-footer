const FULL_WIDTH = { widthMode: "full" };
...
function powerlineLine(segments, options = {}) {
    ...
    if (!first)
        return [];
    if (options.start)
        line.push(capWidget(first.options.bg, options.start, "powerline-start"));
    ...
    if (last && options.end)
        line.push(capWidget(last.options.bg, options.end, "powerline-end"));
    return line;
}
...
export const PRESET_DEFINITIONS = {
    ...
    demo: {
        ...
        iconMode: "nerd",
        terminal: FULL_WIDTH,
        lines: demoLines("pi-footer", "powerline", "powerline-bright", "powerline-blocks", "powerline-mono"),
    },
    "demo-standard": {
        separator: "dot",
        terminal: FULL_WIDTH,
